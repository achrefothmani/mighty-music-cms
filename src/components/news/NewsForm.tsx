...
import { createNews, updateNews, deleteNews, uploadNewsCover } from "@/services/news.service";
...
export default function NewsForm({ initialData }: NewsFormProps) {
  ...
  const handleDelete = async () => {
    if (!initialData || !confirm(`ARE YOU SURE YOU WANT TO DELETE THIS NEWS ARTICLE?`)) return;
    try {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let news: News;
      if (initialData) {
        news = await updateNews(initialData.id, payload);
      } else {
        news = await createNews(payload);
      }

      if (coverFile) {
        await uploadNewsCover(news.id, coverFile, coverCropCoords || undefined);
      }
      ...
      setError(`FAILED TO ${initialData ? "UPDATE" : "CREATE"} NEWS ARTICLE`);
    } finally {
      setIsSubmitting(false);
    }
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div className="space-y-8">
              <div className="space-y-2">
                <label className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">Article Title</label>
                <input name="title" type="text" className="input-brutalist text-xl" defaultValue={initialData?.title} placeholder="E.G. NEW RELEASE ANNOUNCEMENT" required />
              </div>
          ...
          <div className="pt-8 border-t border-border flex justify-end">
            <button type="submit" disabled={isSubmitting} className="btn-primary flex items-center gap-3 px-12 py-4">
              ...
              <span className="text-sm font-bold tracking-[0.1em]">
                {isSubmitting ? "PROCESSING..." : (initialData ? "UPDATE ARTICLE" : "PUBLISH NEWS")}
              </span>
            </button>
          </div>
        </form>
